...
internal class AuthorizeRequestValidator : IAuthorizeRequestValidator
{
        ...
        if (_options.Endpoints.EnableJwtRequestUri)
        {
            if (jwtRequestUri.IsPresent())
            {
                ...
                if (jwt.IsMissing())
                {
                    ...
                    return Invalid(request, OidcConstants.AuthorizeErrors.InvalidRequestUri, "no value returned from request_uri");
                }
    ...
    private async Task<AuthorizeRequestValidationResult> ValidateClientAsync(ValidatedAuthorizeRequest request)
    {
        ...
        {
            if (request.RequestObjectValues.Count is 0)
            {
        ...
        if (responseMode.IsPresent())
        {
            if (Constants.SupportedResponseModes.Contains(responseMode))
            {
                ...
                else
                {
                    ...
                    return Invalid(request, description: "Invalid response_mode for response_type");
                }
            }
            else
            {
    ...
    private async Task<AuthorizeRequestValidationResult> ValidateScopeAsync(ValidatedAuthorizeRequest request)
    {
        ...
        if (validatedResources.Resources.IdentityResources.Count is not 0 && !request.IsOpenIdRequest)
        {
        ...
        if (validatedResources.Resources.ApiScopes.Count is not 0)
        {
        ...
        switch (requirement)
        {
            case Constants.ScopeRequirement.Identity:
                if (validatedResources.Resources.IdentityResources.Count is 0)
                {
            ...
            case Constants.ScopeRequirement.IdentityOnly:
                if (validatedResources.Resources.IdentityResources.Count is 0 || validatedResources.Resources.ApiScopes.Count is not 0)
                {
            ...
            case Constants.ScopeRequirement.ResourceOnly:
                if (validatedResources.Resources.IdentityResources.Count is not 0 || validatedResources.Resources.ApiScopes.Count is 0)
                {
    ...
    private async Task<AuthorizeRequestValidationResult> ValidateOptionalParametersAsync(ValidatedAuthorizeRequest request)
    {
        ...
        if (idp.IsPresent())
        {
            ...
            if (request.Client.IdentityProviderRestrictions != null && request.Client.IdentityProviderRestrictions.Count is not 0)
            {
