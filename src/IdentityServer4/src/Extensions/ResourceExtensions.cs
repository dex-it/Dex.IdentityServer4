...
public static class ResourceExtensions
{
    ...
    internal static ICollection<string> FindMatchingSigningAlgorithms(this IEnumerable<ApiResource> apiResources)
    {
        ...
        var allAlgorithms = apis.Where(r => r.AllowedAccessTokenSigningAlgorithms.Count is not 0).Select(r => r.AllowedAccessTokenSigningAlgorithms).ToList();

        // resources need to agree on allowed signing algorithms
        if (allAlgorithms.Count is not 0)
        {
