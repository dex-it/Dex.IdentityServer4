...
public static class ClientExtensions
{
    ...
    public static bool IsImplicitOnly(this Client client)
        =>
            client is { AllowedGrantTypes.Count: 1 } &&
            client.AllowedGrantTypes.First() is GrantType.Implicit;
    ...
    public static Task<List<SecurityKey>> GetKeysAsync(this IEnumerable<Secret> secrets)
    {
        ...
        var certificates = GetCertificates(secretList)
            .Select(SecurityKey (c) => new X509SecurityKey(c))
            .ToList();

        keys.AddRange(certificates);

        var jwks = secretList
            .Where(s => s.Type is IdentityServerConstants.SecretTypes.JsonWebKey)
    ...
    private static List<X509Certificate2> GetCertificates(IEnumerable<Secret> secrets) => secrets
