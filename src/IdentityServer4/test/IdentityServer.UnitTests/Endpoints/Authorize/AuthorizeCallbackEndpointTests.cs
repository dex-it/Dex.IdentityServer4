...
public class AuthorizeCallbackEndpointTests
{
    ...
    private TestEventService _fakeEventService = new();

    private ILogger<AuthorizeCallbackEndpoint> _fakeLogger = TestLogger.Create<AuthorizeCallbackEndpoint>();

    private IdentityServerOptions _options = new();

    private MockConsentMessageStore _mockUserConsentResponseMessageStore = new();

    private MockUserSession _mockUserSession = new();

    private NameValueCollection _params = new();

    private StubAuthorizeRequestValidator _stubAuthorizeRequestValidator = new();

    private StubAuthorizeResponseGenerator _stubAuthorizeResponseGenerator = new();

    private StubAuthorizeInteractionResponseGenerator _stubInteractionGenerator = new();
    ...
    public async Task ProcessAsync_valid_consent_message_should_cleanup_consent_cookie()
    {
        ...
        var request = new ConsentRequest(parameters, _user.GetSubjectId());
        _mockUserConsentResponseMessageStore.Messages.Add(request.Id, new Message<ConsentResponse>(new ConsentResponse { ScopesValuesConsented =
            ["api1", "api2"]
        }));
