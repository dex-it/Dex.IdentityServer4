...
public class DefaultUserSessionTests
{
    ...
    private MockHttpContextAccessor _mockHttpContext = new();
    private MockAuthenticationHandlerProvider _mockAuthenticationHandlerProvider = new();
    private MockAuthenticationHandler _mockAuthenticationHandler = new();

    private IdentityServerOptions _options = new();
    private ClaimsPrincipal _user;
    private AuthenticationProperties _props = new();
    ...
    public async Task adding_clients_should_be_able_to_read_clients()
    {
        ...
        var clients = await _subject.GetClientListAsync();
        clients.Should().Contain(["client2", "client1"]);
    }
}