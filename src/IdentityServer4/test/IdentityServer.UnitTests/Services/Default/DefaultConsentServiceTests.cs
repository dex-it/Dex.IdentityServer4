...
public class DefaultConsentServiceTests
{
    ...
    private MockProfileService _mockMockProfileService = new();

    private ClaimsPrincipal _user;
    private Client _client;
    private TestUserConsentStore _userConsentStore = new();
    private StubClock _clock = new();
    ...
    public async Task UpdateConsentAsync_when_client_does_not_allow_remember_consent_should_not_update_store()
    {
        ...
        await _subject.UpdateConsentAsync(_user, _client, [new ParsedScopeValue("scope1"), new ParsedScopeValue("scope2")
        ]);
    ...
    public async Task UpdateConsentAsync_empty_scopes_should_should_remove_consent()
    {
        ...
        await _subject.UpdateConsentAsync(_user, _client, []);
    ...
    public async Task RequiresConsentAsync_client_does_not_require_consent_should_not_require_consent()
    {
        ...
        var result = await _subject.RequiresConsentAsync(_user, _client, [new ParsedScopeValue("scope1"), new ParsedScopeValue("scope2")
        ]);
    ...
    {
        var result = await _subject.RequiresConsentAsync(_user, _client, []);
    ...
    {
        var result = await _subject.RequiresConsentAsync(_user, _client, [new ParsedScopeValue("scope1"), new ParsedScopeValue("offline_access")
        ]);
    ...
    {
        await _subject.UpdateConsentAsync(_user, _client, [new ParsedScopeValue("scope1"), new ParsedScopeValue("scope2"), new ParsedScopeValue("scope3")
        ]);

        var result = await _subject.RequiresConsentAsync(_user, _client, [new ParsedScopeValue("scope2")]);
    ...
    {
        await _subject.UpdateConsentAsync(_user, _client, [new ParsedScopeValue("scope2"), new ParsedScopeValue("scope3")
        ]);
