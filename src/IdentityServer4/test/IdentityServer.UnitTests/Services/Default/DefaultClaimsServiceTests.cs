...
public class DefaultClaimsServiceTests
{
    ...
    public ResourceValidationResult ResourceValidationResult => new(_resources);
    ...
    {
        _resources.IdentityResources.Add(new IdentityResource("id_scope", ["foo"]));
    ...
    {
        _resources.IdentityResources.Add(new IdentityResource("id1", ["foo"]));
        _resources.IdentityResources.Add(new IdentityResource("id2", ["bar"]));
        ...
        scopes.ToArray().Should().BeEquivalentTo("api1", "api2", "id1", "id2");
    }
    ...
    public async Task GetAccessTokenClaimsAsync_should_contain_parameterized_scope_values()
    {
        ...
        scopes.ToArray().Should().BeEquivalentTo("api:123");
    }
    ...
    public async Task GetAccessTokenClaimsAsync_should_only_consider_parsed_scope_values_and_not_ApiScope()
    {
        ...
        var scopes = claims.Where(x => x.Type == JwtClaimTypes.Scope).Select(x => x.Value);
        scopes.Count().Should().Be(1);
        scopes.ToArray().Should().BeEquivalentTo("api2");
    }
    ...
    public async Task GetAccessTokenClaimsAsync_when_multiple_resources_with_same_scope_should_contain_scope_once()
    {
        ...
        var scopes = claims.Where(x => x.Type == JwtClaimTypes.Scope).Select(x => x.Value);
        scopes.Count().Should().Be(1);
        scopes.ToArray().Should().BeEquivalentTo("resource");
    }
    ...
    {
        _resources.IdentityResources.Add(new IdentityResource("id1", ["foo"]));
        _resources.ApiResources.Add(new ApiResource("api1", ["bar"]));
    ...
    {
        _resources.ApiResources.Add(new ApiResource("api1", ["foo"]));
        _mockMockProfileService.ProfileClaims.Add(new Claim("aud", "bar"));
