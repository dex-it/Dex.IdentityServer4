...
public class JwtRequestAuthorizeTests
{
    ...
    private const string SymmetricJwk = """
                                        {
                                          "kty": "oct",
                                          "use": "sig",
                                          "kid": "1",
    ...
    private static string CreateRequestJwt(string issuer, string audience, SigningCredentials credential,
        Claim[] claims, bool setJwtTyp = false)
    {
    ...
    public async Task authorize_should_accept_valid_JWT_request_object_parameters_using_X509_certificate()
    {
        var requestJwt = CreateRequestJwt(
            ...
            new X509SigningCredentials(TestCert.Load()),
            [
    ...
    public async Task authorize_should_accept_valid_JWT_request_object_parameters_using_symmetric_jwk()
    {
        var requestJwt = CreateRequestJwt(
            _client.ClientId,
            IdentityServerPipeline.BaseUrl,
            new SigningCredentials(new Microsoft.IdentityModel.Tokens.JsonWebKey(SymmetricJwk), "HS256"),
            [
                new Claim("client_id", _client.ClientId),
                new Claim("response_type", "id_token"),
                new Claim("scope", "openid profile"),
    ...
    public async Task authorize_should_accept_valid_JWT_request_object_parameters_using_rsa_jwk()
    {
        var requestJwt = CreateRequestJwt(
            _client.ClientId,
            IdentityServerPipeline.BaseUrl,
            new SigningCredentials(_rsaKey, "RS256"),
            [
                new Claim("client_id", _client.ClientId),
                new Claim("response_type", "id_token"),
                new Claim("scope", "openid profile"),
    ...
    public async Task authorize_should_accept_complex_objects_in_request_object()
    {
        ...
        // эту проверку пока убираю их тестов (после обновления на net8)
        
        // Microsoft.IdentityModel.JsonWebTokens не восстанавливает ValueType=JsonArray при чтении токена.
        // Массивы claim'ов сериализуются правильно, но при разборе превращаются в несколько claim'ов с типом string.
        // Чтобы сохранить структуру массива, необходимо читать payload напрямую и парсить JSON вручную.
