...
public class DiscoveryEndpointTests
{
    ...
    public async Task Issuer_uri_should_be_lowercase()
    {
        ...
        var result =
            await pipeline.BackChannelClient.GetAsync("HTTPS://SERVER/ROOT/.WELL-KNOWN/OPENID-CONFIGURATION", TestContext.Current.CancellationToken);
    ...
    public async Task Algorithms_supported_should_match_signing_key()
    {
        ...
        var pipeline = new IdentityServerPipeline { EnableRotator = false };
    ...
    public async Task Jwks_entries_should_countain_crv()
    {
        ...
        var result =
            await pipeline.BackChannelClient.GetAsync("https://server/root/.well-known/openid-configuration/jwks", TestContext.Current.CancellationToken);
